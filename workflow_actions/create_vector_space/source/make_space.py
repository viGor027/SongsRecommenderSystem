...
for song in os.listdir(SONGS_DIR):
    ...
    titles_temp, X_temp, Y_temp = prepare_for_dataset(
        data, shuffle=False, return_titles=True
    )
    ...
    with torch.no_grad():
        ...
        specs.append(
            forward_conv_lstm(X_temp[(len(X_temp) // batch_size) * batch_size :])
        )
