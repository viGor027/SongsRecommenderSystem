...
gcs_checkpoint_callback = GCSModelCheckpoint(
    ...
    folder_name="test_cloud_3",
    checkpoint_name="test_cloud_3_ckpt"
)
...
model.init_conv(ConvCls=Conv1DBlockNoDilationNoSkip,
                n_blocks=n_blocks,
                n_layers_per_block=[0 for _ in range(n_blocks)],
                n_filters_per_block=[64+8*i for i in range(n_blocks)],
                n_filters_per_skip=no_skip_filters,
                reduction_strat='conv',
                input_len=216, n_input_channels=80)
...
X_valid = torch.load(os.path.join(DATA_DIR, 'X_valid.pt'))
Y_valid = torch.load(os.path.join(DATA_DIR, 'Y_valid.pt'))
print(X_valid.shape)
